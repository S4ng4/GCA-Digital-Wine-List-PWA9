...
const DeviceDetector = {
    ...
    // Check if device is Samsung Galaxy (including Fold)
    isSamsungGalaxy() {
        const userAgent = navigator.userAgent || navigator.vendor || window.opera;
        return /Samsung|SM-[A-Z]|Galaxy/i.test(userAgent);
    },
    
    // Check if device needs reduced map zoom (phones only, not tablets)
    isReducedZoomPhone() {
        return this.isIPhone() || this.isSamsungGalaxy();
    },
...
function initInteractiveMap() {
        ...
        function initializeMobileMap() {
            ...
            setTimeout(() => {
                ...
                try {
                    const isReducedZoomPhone = DeviceDetector.isReducedZoomPhone();
                    const mobileDefaultZoom = isReducedZoomPhone ? 5.3 : 5.8;
                    const mobileMaxZoom = 5.8;
                    const mobileInitialZoom = mobileDefaultZoom;
                    ...
                    fetch('https://raw.githubusercontent.com/openpolis/geojson-italy/master/geojson/limits_IT_regions.geojson')
                        ...
                        .then(geojson => {
                            ...
                            // Fit map to Italy, then force zoom so user sees the requested zoom level
                            const mobileItalyBounds = mobileGeoJsonLayer.getBounds();
                            mobileMapInstance.fitBounds(mobileItalyBounds, {
                                paddingTopLeft: [8, 16],
                                paddingBottomRight: [8, 12],
                                maxZoom: mobileDefaultZoom,
                                animate: false
                            });
                            const mobileCenter = mobileItalyBounds.getCenter();
                            mobileMapInstance.setView([mobileCenter.lat, mobileCenter.lng], mobileDefaultZoom, { animate: false });
                            ...
                            // Ricalcola dimensioni container quando layout è stabile
                            setTimeout(() => {
                                if (mobileMapInstance && mobileGeoJsonLayer) {
                                    ...
                                    const c = b.getCenter();
                                    mobileMapInstance.setView([c.lat, c.lng], mobileDefaultZoom, { animate: false });
                                }
                            }, 800);
                            scheduleViewportRefresh();
                        })
